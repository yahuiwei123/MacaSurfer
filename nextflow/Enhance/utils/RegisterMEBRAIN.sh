...
Usage() {
  ...
  echo "                [--device=<GPU or CPU use to register>]"
}
...
T1wRestoreBasename=$(remove_ext_bash $T1wRestore)
T1wRestoreBrainBasename=$(remove_ext_bash $T1wRestoreBrain)

echo " "
echo " START: AtlasRegistration to MEBRAIN"
...
# Linear then non-linear registration to MEBRAIN
${UTILS_PATH}/generalRegister.sh -i ${T1wRestoreBrain} -o ${WD}/xfms/ -w ${WD}/orig2temp/ -r ${ReferenceBrain} -m nonlinear -l MI -f aladin -d ${DEVICE}
CreateJacobianDeterminantImage 3 ${WD}/xfms/nonlinear.nii.gz ${WD}/xfms/NonlinearRegJacobians.nii.gz 1
mv ${WD}/xfms/linear.mat ${WD}/xfms/orig2standardLinear.mat
mv ${WD}/xfms/nonlinear.nii.gz ${OutputTransform}

# solve inverse transform
${FSLDIR}/bin/invwarp -w ${OutputTransform} -o ${OutputInvTransform} -r ${T1wRestoreBrain}
...
if [ -e ${T2wImage} ] && [ -e ${T2wRestore} ] && [ -e ${T2wRestoreBrain} ]; then
...
echo " "
echo " END: AtlasRegistration to MEBRAIN"
