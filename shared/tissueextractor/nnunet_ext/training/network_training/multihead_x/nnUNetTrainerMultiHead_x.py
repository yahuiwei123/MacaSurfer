...
from nnunet.network_architecture.generic_UNet2 import Generic_UNet2
from batchgenerators.utilities.file_and_folder_operations import *
from nnunet.training.loss_functions.dice_loss import DC_and_CE_loss
from nnunet_ext.run.default_configuration import get_default_configuration
from nnunet.training.network_training.nnUNetTrainerV2 import nnUNetTrainerV2
from nnunet.training.network_training.nnUNet_variants.data_augmentation.nnUNetTrainerV2_DA3 import nnUNetTrainerV2_DA3_BN_UNeXt,nnUNetTrainerV2_DA3_BN_UNeXt_axial_attn, nnUNetTrainerV2_DA3_BN_UNeXt_GN
...
class nnUNetTrainerMultiHead_x(nnUNetTrainerV2_DA3_BN_UNeXt_axial_attn): # Inherit default trainer class for 2D, 3D low resolution and 3D full resolution U-Net 
#class nnUNetTrainerMultiHead(nnUNetTrainerV2): # Inherit default trainer class for 2D, 3D low resolution and 3D full resolution U-Net 
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.fold = fold
        print("ffffffffffffffffffffffff",fold)
    ...
    def initialize_network(self):
        ...
        if self.trainer_path is None:
            ...
            if self.use_vit:
                ...
                self.mh_network = MultiHead_Module(Generic_UNet2, self.split, self.tasks_list_with_char[0][0], prev_trainer=self.network,
                                                   input_channels=self.num_input_channels, base_num_features=self.base_num_features,\
                                                   num_classes=self.num_classes, num_pool=len(self.net_num_pool_op_kernel_sizes))
                
            # -- Load the correct plans file again, ie. the one from the current task -- #
            # self.process_plans(load_pickle(self.plans_file))
            self.process_plans(load_pickle('../nnUNet_preprocessed/Task509_tissue_infant/nnUNetPlansv2.1_plans_3D.pkl'))
            ...
            #print("self.already_trained_on:",self.already_trained_on[str("3")]['used_split'])
            #print("self.already_trained_on:",self.already_trained_on[str(self.fold)]['used_split'],self.fold)
            self.already_trained_on[str("1")]['used_split'] = self.mh_network.split
            # self.already_trained_on[str(self.fold)]['used_split'] = self.mh_network.split
            #self.already_trained_on[str(self.fold)]['used_split'] = 'seg_outputs'
        ...
        if self.trainer_model.__class__.__name__ == nnUNetTrainerV2.__name__:   # Important when doing evaluation, since nnUNetTrainerV2 has no mh_network
            self.mh_network = MultiHead_Module(Generic_UNet2, self.split, self.tasks_list_with_char[0][0], prev_trainer=self.trainer_model.network,
                                               input_channels=self.num_input_channels, base_num_features=self.base_num_features,\
                                               num_classes=self.num_classes, num_pool=len(self.net_num_pool_op_kernel_sizes))
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True,
                 ...
                 validation_folder_name: str = 'hahaha', debug: bool = False, all_in_gpu: bool = False,
        ...
        print("trained_ontrained_ontrained_ontrained_on:",trained_on_folds)
        ...
        for idx, task in enumerate(trained_on):
            ...
            print("validation_folder_name:",validation_folder_name,validation_folder_name+task)
    ...
    def load_checkpoint_ram(self, checkpoint, train=True, network_old=False, checkpoint_old=None):
        ...
        self.mh_network.add_n_tasks_and_activate(self.already_trained_on[str("1")]['tasks_at_time_of_checkpoint'],
                                                  self.already_trained_on[str("1")]['active_task_at_time_of_checkpoint'])
        ...
        #print("self.already_trained_on[str(self.fold)]['tasks_at_time_of_checkpoint'],self.already_trained_on[str(self.fold)]['active_task_at_time_of_checkpoint']",self.already_trained_on[str(self.fold)]['tasks_at_time_of_checkpoint'],
                                                 #self.already_trained_on[str(self.fold)]['active_task_at_time_of_checkpoint'])
