...
def run_training(extension='multihead'):
    ...
    if extension in ['ewc', 'ewc_vit', 'ewc_unet', 'ewc_x', 'ewc_ln', 'ownm1', 'ownm2', 'ownm3', 'ownm4']:
    ...
    print("os.environ[CUDA_VISIBLE_DEVICES] = ",cuda)
    ...
    if extension in ['ewc', 'ewc_vit', 'ewc_ln', 'ewc_x', 'ewc_unet', 'ownm1', 'ownm2', 'ownm3', 'ownm4']:
        # -- Extract ewc_lambda -- #
    ...
    print("fold[0]fold[0]fold[0]fold[0]",fold[0],'all',str(fold[0]) == 'all')
    
    if fold[0] == 'all':
        fold = list(range(5))
        print("We did all",fold)
    else: # change each fold type from str to int
        fold = list(map(int, fold))
        print("We did not do all",fold)
    # fold = list(range(5))
    ...
    reh_args_x = {'samples_per_ds': samples, 'seed': seed, **basic_exts}
    ...
    args_f = {'nnUNetTrainerRW': rw_args, 'nnUNetTrainerMultiHead': basic_exts,
              'nnUNetTrainerFreezedViT': basic_exts, 'nnUNetTrainerEWCViT': ewc_args, 'nnUNetTrainerEWCXXX': ewc_args,
              ...
              'nnUNetTrainerSequential': basic_exts, 'nnUNetTrainerRehearsal': reh_args,'nnUNetTrainerRehearsal_x': reh_args_x,
...
def main_rehearsal_x():
    r"""Run training for Rehearsal Trainer.
    """
    run_training(extension='rehearsal_x')
