...
from nnunet.network_architecture.generic_UNet3 import Generic_UNet3
from nnunet.network_architecture.generic_UNeXt import Generic_UNeXt
...
class nnUNetTrainerV2_DA3_BN(nnUNetTrainerV2_DA3):
    def initialize_network(self):
        ...
        self.network = Generic_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
                                    ...
                                    axial_attention=False, heads=2, dim_heads=8,volume_shape=(96,160,112),no_attention=[4])
        if torch.cuda.is_available():
            self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper

class nnUNetTrainerV2_DA3_BN_UNeXt(nnUNetTrainerV2_DA3):
    def initialize_network(self):
        if self.threeD:
            conv_op = nn.Conv3d
            dropout_op = nn.Dropout3d
        ...
        net_nonlin = nn.LeakyReLU
        net_nonlin_kwargs = {'negative_slope': 1e-2, 'inplace': True}
        self.network = Generic_UNet2(self.num_input_channels, self.base_num_features, self.num_classes,
                                    len(self.net_num_pool_op_kernel_sizes),
                                    self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                    dropout_op_kwargs,
                                    net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                                    self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True, 300, encoder_scale=1,
                                    axial_attention=False, heads=2, dim_heads=8,volume_shape=(96,160,112),no_attention=[3,4])
        if torch.cuda.is_available():
            self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper

class nnUNetTrainerV2_DA3_BN_UNeXt_axial_attn(nnUNetTrainerV2_DA3):
    def initialize_network(self):
        if self.threeD:
            conv_op = nn.Conv3d
            dropout_op = nn.Dropout3d
        ...
        self.network = Generic_UNet2(self.num_input_channels, self.base_num_features, self.num_classes,
                                    ...
                                    net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                                    self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True, 320, encoder_scale=1,
                                    axial_attention=True, heads=2, dim_heads=8,volume_shape=(96,160,112),no_attention=[3,4])
        if torch.cuda.is_available():
            self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper

class nnUNetTrainerV2_DA3_BN_UNeXt_GN(nnUNetTrainerV2_DA3_BN_UNeXt):
    def initialize_network(self):
        """
        ...
        norm_op_kwargs = {'eps': 1e-5, 'affine': True, 'num_groups': 2}
        dropout_op_kwargs = {'p': 0, 'inplace': True}
        net_nonlin = nn.LeakyReLU
        net_nonlin_kwargs = {'negative_slope': 1e-2, 'inplace': True}
        self.network = Generic_UNet2(self.num_input_channels, self.base_num_features, self.num_classes,
...
class nnUNetTrainerV2_DA3_UNeXt_axial_attn(nnUNetTrainerV2_DA3):
    def initialize_network(self):
        if self.threeD:
            conv_op = nn.Conv3d
            dropout_op = nn.Dropout3d
...
class nnUNetTrainerV2_UNeXt_BN_axial_attn(nnUNetTrainerV2):
    def initialize_network(self):
        if self.threeD:
            conv_op = nn.Conv3d
            dropout_op = nn.Dropout3d
...
class nnUNetTrainerV2_DA3_BN_UNeXt_attn(nnUNetTrainerV2_DA3):
    def initialize_network(self):
        if self.threeD:
            conv_op = nn.Conv3d
            dropout_op = nn.Dropout3d
        ...
        net_nonlin = nn.LeakyReLU
        net_nonlin_kwargs = {'negative_slope': 1e-2, 'inplace': True}
        self.network = Generic_UNet3(self.num_input_channels, self.base_num_features, self.num_classes,
                                    len(self.net_num_pool_op_kernel_sizes),
                                    self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                    dropout_op_kwargs,
                                    net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                                    self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True, 320, encoder_scale=1,
                                    axial_attention=True, heads=2, dim_heads=8,volume_shape=(96,160,112),no_attention=[0,3,4])
        if torch.cuda.is_available():
            self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper
