...
from nnunet.network_architecture.deformable_attention_3d import DeformableAttention3D
...
class ConvNeXt_Block(nn.Module):
    ...
    """
    def __init__(self, input_channels, output_channels, drop_path=0.5, layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv3d(input_channels, output_channels, kernel_size=7, padding=3, groups=output_channels) # depthwise conv
        self.norm = LayerNorm(output_channels, eps=1e-6)
...
class Generic_UNet3(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Generic_UNet3, self).__init__()
        ...
        for u in range(num_pool):
            ...
            if self.do_attention:
                if u not in self.no_attention:
                    ...
                    self.axial_attention.append(DeformableAttention3D(dim = 32*2**(num_pool - u),
                                                               dim_head = dim_heads*2**d,
                                                               heads = heads*2**d,
                                                               dropout = 0.,                       # dropout
