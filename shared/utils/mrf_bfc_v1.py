...
def smooth_bias_field(bias: np.ndarray, mask: np.ndarray, base: int = 10, kernel_size: float = 5) -> np.ndarray:
    ...
    # smooth
    orig_bias = mask * gaussian_filter(orig_bias, sigma=kernel_size)
    return mask * orig_bias
...
def main(args):
    ...
    if modality == 'T1':
        ...
        cls_mean = [np.log(intensity) / np.log(base) for intensity in [1e-3, 0.95, 0.55, 0.15]]
    else:
