...
def estimate_bias_field(image, labels, init_mean, kernel_size=(3, 3, 3), soft_kernel=0.4):
    """
    estimate bias field
    """
    ...
    if modality == 'T1':
        ...
        print("Use label index of 2 and 3 to estimate by default.")
        consider = [2, 3]
    ...
    # print(np.max(residual), np.min(residual), np.mean(residual))
...
def mrf_segmentation(image_pad_array: np.ndarray,
    ...
    for iter in range(bias_max_iter):
        ...
        high_freq_bias = estimate_bias_field(curr_array, label_pad_array, cls_mean, kernel_size=kernel, soft_kernel=label_soft_kernel)
...
def main(args):
    ...
    min_kernel = (args.min_kernel,) * 3 if args.min_kernel != None else compute_sigma(reference_image.GetSpacing())
    ...
    if modality == 'T1':
        cls_mean = [np.log(intensity) / np.log(base) for intensity in [1e-3, 0.15, 0.50, 0.95]]
    elif modality == 'T2':
        cls_mean = [np.log(intensity) / np.log(base) for intensity in [1e-3, 0.95, 0.35, 0.15]]
    else:
        cls_mean = None
        print("Preserve original contrast!")
        
    bias = mrf_segmentation(image_pad, label_pad, bias_max_iter=bias_iters, max_kernel=max_kernel, min_kernel=min_kernel, refine_max_iter=refine_iters, label_soft_kernel=label_soft_kernel, init_dist=cls_mean)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_msk", type=str, default=None, help="only calculate bias in this input mask")
    ...
    parser.add_argument("--modality", type=str, default='None', help="T1 or T2 or None")
